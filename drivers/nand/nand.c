 ...
 * Written by jsgood
 */
static int s3c_nand_calculate_ecc(struct mtd_info *mtd, const uchar *dat, 
uchar *ecc_code)
{
    ulong nfcont, nfmecc0, nfmecc1;
 ...
 * Written by jsgood
 */
static int s3c_nand_correct_data(struct mtd_info *mtd, uchar *dat, uchar *
read_ecc, uchar *calc_ecc)
{
    int ret = -1;
    ulong nfestat0, nfestat1, nfmeccdata0, nfmeccdata1, nfmlcbitpt;
    uchar err_type;
    ...
    {
#ifdef FORLINX_DEBUG
        printf("nand type =S3C_NAND_TYPE_MLC.\n");
#endif
        /* MLC: */
...
int s3c_nand_calculate_ecc_8bit(struct mtd_info *mtd, const uchar *dat, uchar *ecc_code)
{
    ulong nfcont, nfm8ecc0, nfm8ecc1, nfm8ecc2, nfm8ecc3;
...
int s3c_nand_correct_data_8bit(struct mtd_info *mtd, uchar *dat, uchar *read_ecc, uchar *calc_ecc)
{
    int ret = -1;
    ulong nf8eccerr0, nf8eccerr1, nf8eccerr2, nfmlc8bitpt0, nfmlc8bitpt1;
    uchar err_type;
...
static int s3c_nand_write_oob_1bit(struct mtd_info *mtd, struct nand_chip *chip,int page)
{
    uint8_t *ecc_calc = chip->buffers->ecccalc;
    int status = 0;
...
static void s3c_nand_write_page_1bit(struct mtd_info *mtd, struct nand_chip *chip,const uint8_t *buf)
{
...
static int s3c_nand_read_page_1bit(struct mtd_info *mtd, struct nand_chip *chip,uint8_t *buf)
{
 ...
 * Written by jsgood
 */
static int s3c_nand_read_page_4bit(struct mtd_info *mtd, struct nand_chip *chip, uint8_t *buf)
{
    int i, stat, eccsize = chip->ecc.size;
    int eccbytes = chip->ecc.bytes;
    int eccsteps = chip->ecc.steps;
 ...
 * Written by jsgood
 */
static void s3c_nand_write_page_4bit(struct mtd_info *mtd, struct nand_chip *chip, const uint8_t *buf)
{
    int i, eccsize = chip->ecc.size;
    int eccbytes = chip->ecc.bytes;
    int eccsteps = chip->ecc.steps;
...
void board_nand_init(struct nand_chip *nand)
{
    ...
    nand->IO_ADDR_R		= (void *)(NFDATA);
    nand->IO_ADDR_W		= (void *)(NFDATA);
    ...
    {
        s3c_nand_hwcontrol(0, NAND_CMD_RESET, NAND_NCE | NAND_CLE | 
NAND_CTRL_CHANGE);
...
#ifdef FORLINX_DEBUG
               		 printf("Nandflash:Type=SLC  ChipName:samsung-K9F2G08U0B or hynix-HY27UF082G2B \n");
#endif
	...
	if((manufac==0xec)&&(dev_id==0xd3))  
	{
			...
			nand_type = S3C_NAND_TYPE_SLC;
                	...
                	nand->ecc.bytes	= 4;  
                  	...
                  	nand->ecc.write_oob = s3c_nand_write_oob_1bit;
                  	nand->ecc.layout = &s3c_nand_oob_64;
			printf("forlinx********Nandflash:Type=SLC  ChipName:samsung-K9K8G08U0E ******************* \n");
