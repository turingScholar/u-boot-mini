...
uint32_t s3c_usbd_dn_addr;
uint32_t remode_wakeup;
...
const uint8_t string_desc0[] =
{
	4, STRING_DESCRIPTOR, LANGID_US_L, LANGID_US_H,
};

const uint8_t string_desc1[] = /* Manufacturer */
{
...
const uint8_t string_desc2[] = /* Product */
{
...
const uint8_t config_full_total[] =
{
...
const uint8_t config_high_total[] =
{
  0x09, 0x07 ,0x20 ,0x00 ,0x01 ,0x01 ,0x00 ,0xC0 ,0x19,
  0x09 ,0x04 ,0x00 ,0x00 ,0x02 ,0xff ,0x00 ,0x00 ,0x00,
...
uint8_t test_pkt [TEST_PKT_SIZE] = {
...
void s3c_usb_check_current_mode(uint8_t *pucMode)
{
	uint32_t tmp;
...
void s3c_usb_print_pkt(uint8_t *pt, uint8_t count)
{
	int i;
...
void s3c_usb_verify_checksum(void)
{
	...
	cs_start = (uint8_t*)otg.dn_addr;
	cs_end = (uint8_t*)(otg.dn_addr+otg.dn_filesize-10);
	checkSum = 0;
	while(cs_start < cs_end) {
		checkSum += *cs_start++;
		if(((uint32_t)cs_start&0xfffff)==0) printf(".");
	}

	dnCS = *(uint16_t *)cs_end;
...
void s3c_usb_set_inep_xfersize(EP_TYPE type, uint32_t pktcnt, uint32_t xfersize)
{
...
void s3c_usb_set_outep_xfersize(EP_TYPE type, uint32_t pktcnt, uint32_t xfersize)
{
	if(type == EP_TYPE_CONTROL)
	{
...
void s3c_usb_write_ep0_fifo(uint8_t *buf, int num)
{
	int i;
	uint32_t Wr_Data=0;
...
void s3c_usb_write_in_fifo(uint8_t *buf, int num)
{
	int i;
	uint32_t data=0;
...
void s3c_usb_read_out_fifo(uint8_t *buf, int num)
{
	int i;
	uint32_t data;
	...
	for (i=0;i<num;i+=4)
	{
		...
		buf[i] = (uint8_t)data;
		buf[i+1] = (uint8_t)(data>>8);
		buf[i+2] = (uint8_t)(data>>16);
		buf[i+3] = (uint8_t)(data>>24);
	}
}
...
void s3c_usb_get_desc(void)
{
	...
	case DEVICE_DESCRIPTOR:
		otg.req_length = (uint32_t)((otg.dev_req.wLength_H << 8) |
...
void s3c_usb_get_status(void)
{
	...
	case  (0x80):	/*device */
		DBG_SETUP1("DEVICE\n");
		get_status.Device=((uint8_t)remode_wakeup<<1)|0x1; /* SelfPowered */
...
#ifdef USB_OTG_DEBUG_SETUP
		s3c_usb_print_pkt((uint8_t *)&otg.dev_req, 8);
#endif
...
void s3c_usb_set_otherspeed_conf_desc(uint32_t length)
{
	...
	if (otg.speed == USB_HIGH)
	{
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 9);
			writel(EPEN_CNAK_EP0_64, S3C_OTG_DIEPCTL0);
			s3c_usb_write_ep0_fifo(((uint8_t *)&config_full)+0, 9);
		}
	    else if(length ==32)
		{
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 32);
			writel(EPEN_CNAK_EP0_64, S3C_OTG_DIEPCTL0);
			s3c_usb_write_ep0_fifo(((uint8_t *)&config_full_total)+0, 32);
	...
	else
	{
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 8);
			writel(EPEN_CNAK_EP0_8, S3C_OTG_DIEPCTL0);
			s3c_usb_write_ep0_fifo(((uint8_t *)&config_high)+0, 8);
		}
	    else if(length ==32)
		{
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 8);
			writel(EPEN_CNAK_EP0_64, S3C_OTG_DIEPCTL0);
			s3c_usb_write_ep0_fifo(((uint8_t *)&config_high_total)+0, 8);
	    }
...
void s3c_usb_transfer_ep0(void)
{
	...
	case EP0_STATE_GD_DEV_0:
		...
		if (otg.speed == USB_HIGH) {
			...
			if (otg.req_length < DEVICE_DESC_SIZE) {
				...
				s3c_usb_write_ep0_fifo(((uint8_t *)&(otg.desc.dev))+0, otg.req_length);
			} else {
				s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, DEVICE_DESC_SIZE);
				s3c_usb_write_ep0_fifo(((uint8_t *)&(otg.desc.dev))+0, DEVICE_DESC_SIZE);
			}
			otg.ep0_state = EP0_STATE_INIT;
		} else {
			...
			if(otg.req_length<FS_CTRL_PKT_SIZE) {
				...
				s3c_usb_write_ep0_fifo(((uint8_t *)&(otg.desc.dev))+0, FS_CTRL_PKT_SIZE);
	...
	case EP0_STATE_GD_DEV_1:
		...
		if(otg.req_length<(2*FS_CTRL_PKT_SIZE)) {
			s3c_usb_write_ep0_fifo(((uint8_t *)&(otg.desc.dev))+FS_CTRL_PKT_SIZE,
	...
	case EP0_STATE_GD_DEV_2:
		...
		writel(EPEN_CNAK_EP0_8, S3C_OTG_DIEPCTL0);
		if(otg.req_length<DEVICE_DESC_SIZE) {
			s3c_usb_write_ep0_fifo(((uint8_t *)&(otg.desc.dev))+(2*FS_CTRL_PKT_SIZE),
	...
	case EP0_STATE_GD_CFG_0:
		...
		if (otg.speed == USB_HIGH)
		{
			...
			{
				s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, otg.req_length);
				s3c_usb_write_ep0_fifo(((uint8_t *)&(otg.desc.config))+0, otg.req_length);
			}
			else
			{
				s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, CONFIG_DESC_TOTAL_SIZE);
				s3c_usb_write_ep0_fifo(((uint8_t *)&(otg.desc.config))+0, CONFIG_DESC_TOTAL_SIZE);
			}
			otg.ep0_state = EP0_STATE_INIT;
		}
		else
		{
			...
			{
				s3c_usb_write_ep0_fifo(((uint8_t *)&(otg.desc.config))+0, FS_CTRL_PKT_SIZE);
	...
	case EP0_STATE_GD_CFG_1:
		...
		if(otg.req_length<(2*FS_CTRL_PKT_SIZE))
		{
			s3c_usb_write_ep0_fifo(((uint8_t *)&(otg.desc.config))+FS_CTRL_PKT_SIZE,
						(otg.req_length-FS_CTRL_PKT_SIZE));
			otg.ep0_state = EP0_STATE_INIT;
		}
		else
	...
	case EP0_STATE_GD_CFG_2:
		...
		{
			s3c_usb_write_ep0_fifo(((uint8_t *)&(otg.desc.config))+(2*FS_CTRL_PKT_SIZE),
						(otg.req_length-2*FS_CTRL_PKT_SIZE));
			otg.ep0_state = EP0_STATE_INIT;
		}
		else
	...
	case EP0_STATE_GD_CFG_3:
		...
		{
			s3c_usb_write_ep0_fifo(((uint8_t *)&(otg.desc.config))+(3*FS_CTRL_PKT_SIZE),
	...
	case EP0_STATE_GD_DEV_QUALIFIER:	/*only supported in USB 2.0*/
		...
		{
			s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, otg.req_length);
			s3c_usb_write_ep0_fifo((uint8_t *)qualifier_desc+0, otg.req_length);
		}
		else
		{
			s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 10);
			s3c_usb_write_ep0_fifo((uint8_t *)qualifier_desc+0, 10);
		}
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GD_OTHER_SPEED_HIGH_1:
		...
		if(otg.req_length==9)
		{
			s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
				writel(EPEN_CNAK_EP0_8, S3C_OTG_DIEPCTL0);
				s3c_usb_write_ep0_fifo(((uint8_t *)&config_high)+8, 1);
				otg.ep0_state = EP0_STATE_INIT;
		}
		else
		{
			s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 8);
				writel(EPEN_CNAK_EP0_8, S3C_OTG_DIEPCTL0);
				s3c_usb_write_ep0_fifo(((uint8_t *)&config_high)+8, 8);
	...
	case EP0_STATE_GD_OTHER_SPEED_HIGH_2:
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 8);
			writel(EPEN_CNAK_EP0_8, S3C_OTG_DIEPCTL0);
			s3c_usb_write_ep0_fifo(((uint8_t *)&config_high)+16, 8);
	...
	case EP0_STATE_GD_OTHER_SPEED_HIGH_3:
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 8);
			writel(EPEN_CNAK_EP0_8, S3C_OTG_DIEPCTL0);
			s3c_usb_write_ep0_fifo(((uint8_t *)&config_high)+24, 8);
			otg.ep0_state = EP0_STATE_INIT;
			break;
	...
	case EP0_STATE_GD_CFG_ONLY_0:
		...
		if (otg.speed == USB_HIGH)
		{
			...
			else
			{
				...
				writel(EPEN_CNAK_EP0_64, S3C_OTG_DIEPCTL0);
				s3c_usb_write_ep0_fifo(((uint8_t *)&(otg.desc.config))+0,
							CONFIG_DESC_SIZE);
			}
			otg.ep0_state = EP0_STATE_INIT;
		}
	...
	case EP0_STATE_GD_IF_ONLY_0:
		...
		if (otg.speed == USB_HIGH)
		{
			...
			if(otg.req_length<INTERFACE_DESC_SIZE)
			{
				s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, otg.req_length);
				writel(EPEN_CNAK_EP0_64, S3C_OTG_DIEPCTL0);
				s3c_usb_write_ep0_fifo(((uint8_t *)&(otg.desc.intf))+0, otg.req_length);
			}
			else
			{
				s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, INTERFACE_DESC_SIZE);
				writel(EPEN_CNAK_EP0_64, S3C_OTG_DIEPCTL0);
				s3c_usb_write_ep0_fifo(((uint8_t *)&(otg.desc.intf))+0, INTERFACE_DESC_SIZE);
			}
			otg.ep0_state = EP0_STATE_INIT;
		}
		else
		{
			...
			{
				s3c_usb_write_ep0_fifo(((uint8_t *)&(otg.desc.intf))+0, FS_CTRL_PKT_SIZE);
	...
	case EP0_STATE_GD_IF_ONLY_1:
		...
		writel(EPEN_CNAK_EP0_8, S3C_OTG_DIEPCTL0);
		s3c_usb_write_ep0_fifo(((uint8_t *)&(otg.desc.intf))+FS_CTRL_PKT_SIZE,
	...
	case EP0_STATE_GD_EP0_ONLY_0:
		...
		s3c_usb_write_ep0_fifo(((uint8_t *)&(otg.desc.ep1))+0, ENDPOINT_DESC_SIZE);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GD_EP1_ONLY_0:
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, ENDPOINT_DESC_SIZE);
		writel(EPEN_CNAK_EP0_8, S3C_OTG_DIEPCTL0);
		s3c_usb_write_ep0_fifo(((uint8_t *)&(otg.desc.ep2))+0, ENDPOINT_DESC_SIZE);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GD_STR_I0:
		...
		writel(EPEN_CNAK_EP0_8, S3C_OTG_DIEPCTL0);
		s3c_usb_write_ep0_fifo((uint8_t *)string_desc0, STRING_DESC0_SIZE);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GD_STR_I1:
		...
		if ((otg.ep0_substate*otg.ctrl_max_pktsize+otg.ctrl_max_pktsize)
			...
			if (otg.speed == USB_HIGH)
				...
				writel(EPEN_CNAK_EP0_8, S3C_OTG_DIEPCTL0);
			s3c_usb_write_ep0_fifo((uint8_t *)string_desc1+(otg.ep0_substate*otg.ctrl_max_pktsize),
	...
	case EP0_STATE_GD_STR_I2:
		...
		if ((otg.ep0_substate*otg.ctrl_max_pktsize+otg.ctrl_max_pktsize)
			...
			if (otg.speed == USB_HIGH)
				...
				writel(EPEN_CNAK_EP0_8, S3C_OTG_DIEPCTL0);
			s3c_usb_write_ep0_fifo((uint8_t *)string_desc2+(otg.ep0_substate*otg.ctrl_max_pktsize),
						otg.ctrl_max_pktsize);
			otg.ep0_state = EP0_STATE_GD_STR_I2;
			otg.ep0_substate++;
		} else {
	...
	case EP0_STATE_INTERFACE_GET:
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
		writel(EPEN_CNAK_EP0_8, S3C_OTG_DIEPCTL0);
		s3c_usb_write_ep0_fifo((uint8_t *)&get_intf+0, 1);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GET_STATUS0:
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
		writel(EPEN_CNAK_EP0_8, S3C_OTG_DIEPCTL0);
		s3c_usb_write_ep0_fifo((uint8_t *)&get_status+0, 1);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GET_STATUS1:
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
		writel(EPEN_CNAK_EP0_8, S3C_OTG_DIEPCTL0);
		s3c_usb_write_ep0_fifo((uint8_t *)&get_status+1, 1);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GET_STATUS2:
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
		writel(EPEN_CNAK_EP0_8, S3C_OTG_DIEPCTL0);
		s3c_usb_write_ep0_fifo((uint8_t *)&get_status+2, 1);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GET_STATUS3:
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
		writel(EPEN_CNAK_EP0_8, S3C_OTG_DIEPCTL0);
		s3c_usb_write_ep0_fifo((uint8_t *)&get_status+3, 1);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GET_STATUS4:
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
		writel(EPEN_CNAK_EP0_8, S3C_OTG_DIEPCTL0);
		s3c_usb_write_ep0_fifo((uint8_t *)&get_status+4, 1);
		otg.ep0_state = EP0_STATE_INIT;
		break;
...
void s3c_usb_int_bulkin(void)
{
	...
	bulkin_buf = (uint8_t*)otg.up_ptr;
	remain_cnt = otg.up_size- ((uint32_t)otg.up_ptr - otg.up_addr);
...
void s3c_usb_upload_start(void)
{
	...
	s3c_usb_read_out_fifo((uint8_t *)tmp_buf, 10);
	check = *((uint8_t *)(tmp_buf+8)) + (*((uint8_t *)(tmp_buf+9))<<8);

	if (check==0x1) {
		otg.up_addr =
			*((uint8_t *)(tmp_buf+0))+
			(*((uint8_t *)(tmp_buf+1))<<8)+
			(*((uint8_t *)(tmp_buf+2))<<16)+
			(*((uint8_t *)(tmp_buf+3))<<24);

		otg.up_size =
			*((uint8_t *)(tmp_buf+4))+
			(*((uint8_t *)(tmp_buf+5))<<8)+
			(*((uint8_t *)(tmp_buf+6))<<16)+
			(*((uint8_t *)(tmp_buf+7))<<24);

		otg.up_ptr=(uint8_t *)otg.up_addr;
		...
		if (otg.op_mode == USB_CPU) {
			...
			writel((uint32_t)otg.up_ptr, S3C_OTG_DIEPDMA_IN);

			pktcnt = (uint32_t)(otg.up_size/otg.bulkin_max_pktsize);
			remainder = (uint32_t)(otg.up_size%otg.bulkin_max_pktsize);
			if(remainder != 0) {
				pktcnt += 1;
			}
...
void s3c_usb_download_start(uint32_t fifo_cnt_byte)
{
	uint8_t tmp_buf[8];

	s3c_usb_read_out_fifo((uint8_t *)tmp_buf, 8);
	...
	if (otg.op_mode == USB_CPU) {
		...
		writel(MODE_DMA|BURST_INCR4|GBL_INT_UNMASK,
			S3C_OTG_GAHBCFG);
		writel((uint32_t)otg.dn_ptr, S3C_OTG_DOEPDMA_OUT);
		pkt_cnt = (uint32_t)(otg.dn_filesize-otg.bulkout_max_pktsize)/otg.bulkout_max_pktsize;
		remain_cnt = (uint32_t)((otg.dn_filesize-otg.bulkout_max_pktsize)%otg.bulkout_max_pktsize);
		if(remain_cnt != 0) {
			pkt_cnt += 1;
		}
...
void s3c_usb_download_continue(uint32_t fifo_cnt_byte)
{
	if (otg.op_mode == USB_CPU) {
		s3c_usb_read_out_fifo((uint8_t *)otg.dn_ptr, fifo_cnt_byte);
		...
		if (((uint32_t)otg.dn_ptr - otg.dn_addr) >= (otg.dn_filesize - 8)) {
...
void s3c_usb_int_bulkout(uint32_t fifo_cnt_byte)
{
...
void s3c_usb_dma_in_done(void)
{
	...
	otg.up_ptr = (uint8_t *)readl(S3C_OTG_DIEPDMA_IN);
	remain_cnt = otg.up_size- ((uint32_t)otg.up_ptr - otg.up_addr);

	if (remain_cnt>0) {
		uint32_t pktcnt, remainder;
		pktcnt = (uint32_t)(remain_cnt/otg.bulkin_max_pktsize);
		remainder = (uint32_t)(remain_cnt%otg.bulkin_max_pktsize);
		if(remainder != 0) {
			pktcnt += 1;
		}
...
void s3c_usb_dma_out_done(void)
{
	...
	otg.dn_ptr = (uint8_t *)readl(S3C_OTG_DOEPDMA_OUT);

	remain_cnt = otg.dn_filesize - ((uint32_t)otg.dn_ptr - otg.dn_addr + 8);

	if (remain_cnt>0) {
		uint32_t pktcnt, remainder;
		pktcnt = (uint32_t)(remain_cnt/otg.bulkout_max_pktsize);
		remainder = (uint32_t)(remain_cnt%otg.bulkout_max_pktsize);
		if(remainder != 0) {
			pktcnt += 1;
		}
...
/* dbg	  descConf.bConfigurationValue=2; // why 2? There's no reason.*/
	...
	otg.desc.ep1.wMaxPacketSizeL=(uint8_t)otg.bulkin_max_pktsize; /* 64*/
	otg.desc.ep1.wMaxPacketSizeH=(uint8_t)(otg.bulkin_max_pktsize>>8);
	...
	otg.desc.ep2.wMaxPacketSizeL=(uint8_t)otg.bulkout_max_pktsize; /* 64*/
	otg.desc.ep2.wMaxPacketSizeH=(uint8_t)(otg.bulkout_max_pktsize>>8);
